...
public class LifespanReport extends BaseReport {
    ...
    public LifespanReport(Context context, Map<Integer, Place> placesInActiveTree, Map<Integer, Individual> individualsInActiveTree, Map<Integer, Family> familiesInActiveTree, List<FamilyChild> familyChildrenInActiveTree, NameFormat nameFormat, IncludedIndividuals includedIndividuals, int maxGenerations, int treeId) {
    ...
    public boolean generateReport(String filename, int activeIndividualId) throws IOException {
        ...
        if (this.individualsInActiveTree.containsKey(activeIndividualId)) {
            ...
            if (includedIndividuals == IncludedIndividuals.Ancestors)
                this.writeLine("Lifespan report for the ancestors of " + AncestryUtil.generateName(individual, this.nameFormat));
            else if (includedIndividuals == IncludedIndividuals.Decendants)
                this.writeLine("Lifespan report for the descendants of " + AncestryUtil.generateName(individual, this.nameFormat));
            else
                this.writeLine("Lifespan report for all individuals in the tree");
    ...
    private void outputReport() throws IOException {
        this.writeLine("Total individuals in report " + String.valueOf(ancestorIds.size()));
        this.writeLine("");
        this.writeLine(String.format("Oldest male (%.2f years old): %s", oldestMaleAge, AncestryUtil.generateName(this.individualsInActiveTree.get(oldestMaleId), nameFormat)));
        this.writeLine(String.format("Oldest female (%.2f years old): %s", oldestFemaleAge, AncestryUtil.generateName(this.individualsInActiveTree.get(oldestFemaleId), nameFormat)));
    ...
    private void getDemographicsFromIndividual(Individual individual) {
        ancestorIds.add(individual.individualId);
        float age = AncestryUtil.calculateAge(individual);

        if (age != -1) {
            individualAges.put(individual.individualId, age);
            ...
            if (individual.gender == GenderEnum.Male) {
                if (age > oldestMaleAge) {
                    oldestMaleAge = age;
                    oldestMaleId = individual.individualId;
                }
                ...
                if (demographicLifespan != null) {
                    ...
                    demographicLifespan.individualIds.add(individual.individualId);
                }
            } else if (individual.gender == GenderEnum.Female) {
                if (age > oldestFemaleAge) {
                    oldestFemaleAge = age;
                    oldestFemaleId = individual.individualId;
                }
    ...
    private void processAncestorsIndividual(int individualId, int generation) {
        ...
        if (!ancestorIds.contains(individualId)) {
            ...
            getDemographicsFromIndividual(individual);
