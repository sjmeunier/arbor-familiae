...
public class MtDNAReport extends BaseReport {

    public MtDNAReport(Context context, AppDatabase database, Map<Integer, Place> placesInActiveTree, Map<Integer, Individual> individualsInActiveTree, Map<Integer, Family> familiesInActiveTree, List<FamilyChild> familyChildrenInActiveTree, NameFormat nameFormat, int maxGenerations, int treeId) {
    ...
    public boolean generateReport(String filename, int activeIndividualId) throws IOException {
        ...
        this.writeLine("Descendants of " + AncestryUtil.generateName(individual, this.nameFormat) + " sharing the same mt-DNA");
        ...
        if (this.individualsInActiveTree.containsKey(individualId)) {
            ...
            if (parentFamilyId != -1 && this.familiesInActiveTree.containsKey(parentFamilyId)) {
                ...
                if (family.wifeId != -1 && this.individualsInActiveTree.containsKey(family.wifeId)) {
                    rootIndividualId = findRootPerson(family.wifeId);
                }
            }
        }
    ...
    private void processPerson(int individualId, int generation) throws IOException  {
        ...
        if (individual.gender == GenderEnum.Female) {
